...
CRON_CMD="$SCHEDULE cd $REPO_ROOT && $SYNC_SCRIPT >> /tmp/gdocs-md-sync.log 2>&1 $CRON_MARKER"
...
case "$ACTION" in
  install)
    ...
    echo "  Log: /tmp/gdocs-md-sync.log"
    ;;
  remove)
    CURRENT="$(crontab -l 2>/dev/null || true)"
    FILTERED="$(echo "$CURRENT" | grep -v "$CRON_MARKER" || true)"
    if [[ "$FILTERED" != "$CURRENT" ]]; then
      ...
      echo "No gdocs-md-sync cron job found."
    fi
    ;;
  status)
    ...
    if [[ -n "$ENTRY" ]]; then
      ...
      echo "No gdocs-md-sync cron job installed."
    fi
    ;;
esac
