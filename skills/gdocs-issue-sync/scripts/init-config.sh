...
cat > "$CONF_FILE" <<'CONF'
# Google Docs Issue Sync configuration
...
# Emoji markers in tab title that indicate completed/resolved issues
...
if [[ -f "$REPO_ROOT/.gitignore" ]]; then
  if ! grep -qF '.gdocs-issue-sync.conf' "$REPO_ROOT/.gitignore"; then
    echo "Tip: Add .gdocs-issue-sync.conf to your .gitignore:"
    echo "  echo '.gdocs-issue-sync.conf' >> $REPO_ROOT/.gitignore"
  fi
else
  echo "Tip: Add .gdocs-issue-sync.conf to your .gitignore to keep config out of version control."
fi
